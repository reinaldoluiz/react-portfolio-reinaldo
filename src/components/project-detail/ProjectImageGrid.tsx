...
const ProjectImageGrid = ({ project }: ProjectImageGridProps) => {
  const renderGrid = () => {
    ...
    if (project.id === 1) {
      return <SingleImageGrid image={resolveImagePath(project.image)} title={project.title} />;
    }
    ...
    if (fourImageGridProjects.includes(project.id) && project.customImages) {
      return <Project4Grid images={project.customImages.map(resolveImagePath)} />;
    }
    // Project 5: 3-image grid
    if (project.id === 5 && project.customImages) {
      return <Project5Grid images={project.customImages.map(resolveImagePath)} />;
    }
